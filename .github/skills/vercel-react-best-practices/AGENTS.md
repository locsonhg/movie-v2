...
9. [Component Architecture](#9-component-architecture) — **HIGH** ⚡ ENFORCED
   - 9.1 [One Component Per File](#91-one-component-per-file)
   - 9.2 [One Responsibility Per File](#92-one-responsibility-per-file)
   - 9.3 [Feature-Based Folder Structure](#93-feature-based-folder-structure)
   - 9.4 [Folder Index File](#94-folder-index-file)
...
**Impact: HIGH** ⚡ **ENFORCED — Always apply these rules when generating or modifying code.**

Maintainability and scalability depend on clear separation of concerns. These rules are mandatory for this project.

### 9.1 One Component Per File

**Impact: HIGH (separation of concerns, easier navigation and testing)**

Each file must export exactly one primary React component. Tightly coupled private sub-components may coexist but must **not** be exported.
...
**Impact: HIGH (prevents mixing concerns, makes files predictable)**

A file must handle exactly one concern: a component, a hook, a service, a type definition, or a utility. Never mix them.
...
export async function fetchMovie() {}        // API call
...
**Impact: HIGH (co-location, easier to find and change related code)**

Organize components by **feature/domain**, not by technical type.

**Incorrect: organized by type**

```
components/
  cards/       ← all cards mixed together
  grids/       ← all grids mixed together
  modals/      ← all modals mixed together
```
...
### 9.4 Folder Index File

**Impact: MEDIUM (clean imports, easier refactoring)**

Each feature folder must have an `index.ts` that re-exports its public API.
